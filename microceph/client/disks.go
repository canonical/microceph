...
func AddDisk(ctx context.Context, c mcTypes.Client, data *types.DisksPost) (types.DiskAddResponse, error) {
	queryCtx, cancel := context.WithTimeout(ctx, diskAddTimeout)
	defer cancel()

	errors := types.DiskAddResponse{}
	err := c.Query(queryCtx, "POST", types.ExtendedPathPrefix, &api.NewURL().Path("disks").URL, data, &errors)
	if err != nil {
...
func GetDisks(ctx context.Context, c mcTypes.Client) (types.Disks, error) {
	queryCtx, cancel := context.WithTimeout(ctx, time.Second*5)
	defer cancel()

	disks := types.Disks{}

	err := c.Query(queryCtx, "GET", types.ExtendedPathPrefix, &api.NewURL().Path("disks").URL, nil, &disks)
	if err != nil {
...
func GetResources(ctx context.Context, c mcTypes.Client) (*api.ResourcesStorage, error) {
	queryCtx, cancel := context.WithTimeout(ctx, time.Second*5)
	defer cancel()

	storage := api.ResourcesStorage{}

	err := c.Query(queryCtx, "GET", types.ExtendedPathPrefix, &api.NewURL().Path("resources").URL, nil, &storage)
	if err != nil {
...
func RemoveDisk(ctx context.Context, c mcTypes.Client, data *types.DisksDelete) error {
	...
	err = c.Query(queryCtx, "DELETE", types.ExtendedPathPrefix, &api.NewURL().Path("disks", strconv.FormatInt(data.OSD, 10)).URL, data, nil)
	if err != nil {
