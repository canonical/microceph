...
func setupMonHostConfigDB(t *testing.T) *sql.DB {
	...
	applyMonHostConfigSchemaUpdate(t, db)
	return db
}

func newMonHostConfigDB(t *testing.T) *sql.DB {
	t.Helper()
...
func applyMonHostConfigSchemaUpdate(t *testing.T, db *sql.DB) {
	t.Helper()
...
INSERT INTO config (key, value) VALUES
  ('mon.host.node-a', '10.0.0.1'),
  ('mon.host.removed-node', '10.0.0.2'),
  ('mon.host.1', '192.0.2.1'),
  ('mon.host.18446744073709551616', '192.0.2.2'),
  ('other.key', 'unchanged');
`)
	require.NoError(t, err)
