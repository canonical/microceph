...
func TestRgwOverwriteRequestType(t *testing.T) {
	// Promote and demote share one endpoint and one HTTP verb, so the route
	// cannot say which of the two arrived. The server passes the empty
	// sentinel to mean "trust the body", and the guard has to leave the
	// client's value alone.
	req := RgwReplicationRequest{RequestType: PromoteReplicationRequest}
	req.OverwriteRequestType(WorkloadReplicationRequest)
	assert.Equal(t, PromoteReplicationRequest, req.RequestType)

	// Every other endpoint maps to exactly one verb, so the server's value
	// wins over whatever the client encoded.
