...
const (
	...
	// local zone's place in the multisite topology.
	RgwResourceSite RgwResourceType = constants.RgwSiteResource
	// RgwResourceBucket scopes a request to a single bucket.
	RgwResourceBucket RgwResourceType = "bucket"
)

// RgwReplicationRequest implements ReplicationRequest for RGW replication.
// Every verb shares this one struct, so a request built for a cluster wide
// operation leaves most of it zero valued.
type RgwReplicationRequest struct {
	RemoteName string `json:"remote" yaml:"remote"`
	Bucket     string `json:"bucket" yaml:"bucket"`
	// Mode is the zonegroup scoped replication mode: symmetrical,
	// directional or selective.
	Mode string `json:"mode" yaml:"mode"`
	// BucketMode is the bucket scoped replication mode: symmetrical,
	// directional, disabled or inherit.
	BucketMode       string                 `json:"bucket_mode" yaml:"bucket_mode"`
	LocalEndpoints   string                 `json:"endpoints" yaml:"endpoints"`
	RemoteEndpoints  string                 `json:"remote_endpoints" yaml:"remote_endpoints"`
	RefreshEndpoints bool                   `json:"refresh_endpoints" yaml:"refresh_endpoints"`
