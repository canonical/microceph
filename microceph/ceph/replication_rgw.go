...
//
// Everything it reports is read live from RADOS, never from the database.
// That is deliberate: a secondary cluster stores no replication state of its
// own, so a database backed answer would be empty exactly where an operator
// most needs one.
